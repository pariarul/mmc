...
export default function AdminAnnouncementsPage() {
  ...
  const handleAdd = async (e) => {
    ...
    setAdding(true);
    try {
      const response = await axios.post('/api/announcements', form);
      if (response.data?.success) {
        showToast('Circular announcement posted.', 'success');
  ...
  const handleDelete = async (id) => {
    if (!window.confirm('Are you sure you want to delete this announcement circular? It will instantly be removed from the public announcements dashboard.')) {
      return;
    }
      ...
      <div className="w-full bg-govBlue-900 border-b border-govBlue-800 px-6 py-4 flex items-center gap-3">
        ...
        <div>
          <h2 className="text-lg md:text-xl font-black uppercase tracking-tight flex items-center gap-2">
            <FileText className="w-5 h-5 text-govGold-500" />
            Bulletins Editor
          </h2>
          <p className="text-[10px] text-govGold-400 font-bold uppercase tracking-wider">Manage Circular Bulletins Board</p>
        </div>
      </div>
        ...
        <div className="lg:col-span-5">
          <div className="bg-govBlue-900 border border-govBlue-850 rounded-3xl p-6 shadow-2xl">
            <h3 className="font-extrabold text-sm uppercase text-govGold-400 mb-6 border-b border-govBlue-850 pb-2.5">
              Add New Circular Bulletin
            </h3>

            <form onSubmit={handleAdd} className="flex flex-col gap-4 text-xs">
              <div className="flex flex-col gap-1.5">
                <label className="font-bold text-govBlue-200">Bulletin Title (Publicly Displayed) *</label>
                <input
                  type="text"
                  required
                  value={form.title}
                  onChange={(e) => setForm(prev => ({ ...prev, title: e.target.value }))}
                  placeholder="e.g. FMG Internship rotation schedule for June 2026 batches"
              ...
              <div className="flex flex-col gap-1.5">
                <label className="font-bold text-govBlue-200">PDF Circular File URL *</label>
                <input
                  type="text"
                  required
                  value={form.file_url}
                  onChange={(e) => setForm(prev => ({ ...prev, file_url: e.target.value }))}
                  placeholder="Supply dynamic PDF link or use standard default placeholder"
                  className="bg-govBlue-955 border border-govBlue-800 rounded-lg p-3 text-white focus:outline-hidden focus:border-govGold-500"
                />
              </div>
              ...
              >
                ...
                {adding ? 'Posting Bulletin...' : 'Post Circular Bulletin'}
              </button>
            </form>
          </div>
        </div>
        ...
        <div className="lg:col-span-7">
          <div className="bg-govBlue-900 border border-govBlue-850 rounded-3xl p-6 shadow-2xl flex-grow flex flex-col">
            <h3 className="font-black text-sm uppercase text-govGold-400 border-b border-govBlue-850 pb-2.5 mb-4">
              Published Circular Bulletins
            </h3>

            {loading ? (
              <div className="text-center py-12 text-xs text-govBlue-400">Loading circulars registry...</div>
                ...
                {announcements.map((item) => (
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <div className="p-2.5 rounded-lg bg-govBlue-900 text-red-500 border border-govBlue-800">
                        <FileText className="w-5 h-5" />
                      </div>
                      <div>
                        <h4 className="font-extrabold text-xs text-white leading-snug line-clamp-1 pr-4">{item.title}</h4>
                        <p className="text-[10px] text-govBlue-400 mt-1 uppercase font-bold">PDF Attachment circular</p>
                      </div>
                    </div>
                    <button
              ...
              <div className="text-center py-12 text-xs text-govBlue-400">
                No circular announcements posted yet.
              </div>
            )}
          </div>
        </div>
